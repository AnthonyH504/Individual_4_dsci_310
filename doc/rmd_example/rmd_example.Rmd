...
```{r horse-population, fig.cap='Horse populations for all provinces in Canada from 1906 - 1972', fig.dim = c(8, 6), echo=FALSE}
...
knitr::kable(horses_sd_table, caption = "sample standard deviation of the number of horses")
```
...
Additionally, note that in Table \@ref(tab:table) we consider the sample standard deviation of the number of horses during the same time span as Figure \@ref(fig:horse-population).

```{r horsesdplot, fig.cap='Horse populations for the province with the largest standard deviation', fig.dim = c(8, 6), echo = FALSE}
knitr::include_graphics("../../results/horse_pop_plot_largest_sd.png")
```

In Figure \@ref(fig:horsesdplot) we zoom in and look at the province of `r largest_sd`, which had the largest spread of values in terms of standard deviation.
